...
class HybridEncoderLayerWithFrequencyAdaptiveNormAndCrossAttn(nn.Module):
    """Hybrid encoder layer with frequency attention, adaptive normalization, and cross-attention"""
    def __init__(self, self_attention, cross_attention, d_model, d_ff=None,
                 dropout=0.1, activation="gelu", n_heads=8):
        super(HybridEncoderLayerWithFrequencyAdaptiveNormAndCrossAttn, self).__init__()
        d_ff = d_ff or 4 * d_model
        self.self_attention = self_attention
        self.cross_attention = cross_attention
        
        # Frequency-enhanced attention
        self.freq_attention = FrequencyEnhancedAttention(d_model, n_heads=n_heads, dropout=dropout)
        
        # Enhanced feed-forward
        self.conv1 = nn.Conv1d(in_channels=d_model, out_channels=d_ff, kernel_size=1)
        self.conv2 = nn.Conv1d(in_channels=d_ff, out_channels=d_model, kernel_size=1)
        
        # Adaptive normalization (replaces LayerNorm)
        self.norm1 = AdaptiveNormalization(d_model)
        self.norm2 = AdaptiveNormalization(d_model)
        self.norm3 = nn.LayerNorm(d_model)  # Keep standard LayerNorm for final normalization
    ...
    def forward(self, x, cross=None, x_mask=None, cross_mask=None, tau=None, delta=None):
        ...
        return self.norm3(x + y)  # Standard LayerNorm for final normalization
...
class Model(nn.Module):
    """
    PatchXFormer Variant 5: Full Model (+Residual Prediction Head)
    
    This is the FULL MODEL with ALL enhancements:
    ...
    def __init__(self, configs, patch_len=16, stride=8):
        ...
        # Exogenous embedding (used in cross-attention)
        ...
        # Hybrid encoder layers with frequency attention, adaptive normalization, and cross-attention
        self.encoder = nn.ModuleList([
            HybridEncoderLayerWithFrequencyAdaptiveNormAndCrossAttn(
                AttentionLayer(
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        # Hybrid encoder processing with frequency attention, adaptive norm, and cross-attention
        for layer in self.encoder:
            enc_out = layer(enc_out, ex_embed)  # Pass ex_embed for cross-attention
    ...
    def anomaly_detection(self, x_enc):
        # Same as forecast (no exogenous features for anomaly detection)
        means = x_enc.mean(1, keepdim=True).detach()
        x_enc = x_enc - means
        stdev = torch.sqrt(torch.var(x_enc, dim=1, keepdim=True, unbiased=False) + 1e-5)
        x_enc /= stdev
        ...
        # No exogenous features for anomaly detection
        for layer in self.encoder:
            enc_out = layer(enc_out, cross=None)
